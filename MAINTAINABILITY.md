...
- [ ] `MovieListResponse`/`TvListResponse` type `results` as `(TvShow & Movie)[]` — an intersection that's structurally impossible for a real response to satisfy (a movie result can't have `TvShow`'s required `name`). Should be a `media_type`-discriminated union instead — the field already exists on both base types but is never used to discriminate. This will matter more once the dedicated Movies/TV Shows pages consume these types directly (each page's results are honestly single-type, but the type system won't reflect that until this is fixed).
